#!/usr/bin/perl
# AoC2022 day 04 - camp cleanup - puzzle star 1 + 2
...
my $numendover=0; # only one-sided overlapping ranges
# loop over file
...
while(<FH>){
  ...
  if(m/(\d+)-(\d+),(\d+)-(\d+)/){ # expected format
    ...
    if($embeds==1){ # disjoint counters
      $numembed+=$embeds;
    # one-sided overlap == one point from a is inside range b 
    }elsif(ptInRange($a0, $b0,$b1) || ptInRange($a1, $b0,$b1)) { 
      $numendover++; 
      print "single-side overlap";
    }
  }else{ # no regex match:
...
print "overall count of somehow-overlapping ranges in pairs = ".($numendover+$numembed).".\n";
